<template>
  <div class="article-wrap" v-if="servicio">
    ...
    <div class="article-breadcrumb">
      ...
      <span class="sep">/</span>
      <span class="breadcrumb-current">{{ servicio.breadcrumbLabel }}</span>
    </div>

    <article class="article-body">
      <header class="article-header">
        <div class="article-meta">
          <span class="article-season-badge" :style="{ color: servicio.color, background: servicio.colorPale, border: `1px solid ${servicio.color}33` }">
            <font-awesome-icon :icon="servicio.icon" class="me-1" /> {{ servicio.tag }}
          </span>
        </div>
        <h1 class="article-title">{{ servicio.title }}</h1>
        <p class="article-excerpt">{{ servicio.excerpt }}</p>
      </header>

      <div class="article-hero-img" v-if="servicio.image">
        <img :src="servicio.image" :alt="servicio.title" loading="eager" />
      </div>

      <div class="article-content" v-html="purify(servicio.content)"></div>

      <div class="article-cta">
        <div class="article-cta-inner">
          <h2 class="article-cta-title">¿Necesitas este servicio?</h2>
      ...
      <div class="article-related" v-if="related.length">
        <h3 class="related-title">Otros servicios</h3>
        <div class="related-grid">
          <router-link
            ...
            class="related-card"
          >
            <span class="related-icon" :style="{ background: s.colorPale, color: s.color }">
              <font-awesome-icon :icon="s.icon" />
            </span>
            <div class="related-text">
              <span class="related-season" :style="{ color: s.color }">{{ s.tag }}</span>
              <p class="related-card-title">{{ s.title }}</p>
              <span class="related-arrow" :style="{ color: s.color }">
                Ver <font-awesome-icon :icon="['fas', 'arrow-right']" class="ms-1" />
              </span>
            </div>
          </router-link>
        </div>
...
import { servicios, getServicioBySlug } from '../data/servicios.js';
import DOMPurify from 'dompurify';

const purify = (html) => DOMPurify.sanitize(html, { USE_PROFILES: { html: true } });

const route = useRoute();
const servicio = computed(() => getServicioBySlug(route.meta?.servicio));
// Rota circularmente (siguiente 3 del array) en vez de tomar siempre los 3
// primeros: así cada servicio recibe enlaces internos desde otras 3 páginas
// en vez de que los últimos del array queden huérfanos de enlaces.
const related = computed(() => {
  const idx = servicios.findIndex((s) => s.slug === route.meta?.servicio);
  if (idx === -1) return servicios.slice(0, 3);
  const n = servicios.length;
  return [1, 2, 3].map((offset) => servicios[(idx + offset) % n]);
});

// El HTML estático (generado en build) ya incluye el schema Service+Breadcrumb+FAQ
// correcto para esta URL. Esto solo se usa para mantenerlo sincronizado si el
// usuario navega de un servicio a otro sin recargar la página (navegación SPA):
// borra cualquier schema dinámico previo (el del build o el de otro servicio)
// y pone el que corresponde al servicio actual, evitando duplicados.
function injectSchema(s) {
  document.querySelectorAll('script[data-dynamic-schema]').forEach((el) => el.remove());
  if (!s) return;
  const url = 'https://royallclean.es/' + s.slug;
    ...
    '@graph': [
      {
        ...
        provider: { '@id': 'https://royallclean.es/#business' },
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: s.breadcrumbLabel, item: url },
        ],
      },
      {
        '@type': 'FAQPage',
...
onMounted(() => injectSchema(servicio.value));
watch(servicio, injectSchema);
